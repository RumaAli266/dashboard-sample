...
import { menuOpen, setMenu } from '../../../../../features/customization/customizationSlice';

// ==============================|| SIDEBAR MENU LIST ITEMS ||============================== //

const NavItem : React.FC<any> = ({ item, level }) => {
  ...
  const itemIcon = item?.icon ? (
    ...
    <FiberManualRecordIcon
      sx={{
        width: customization.isOpen.findIndex((id:number) => id === item?.id) > -1 ? 8 : 6,
  ...
  listItemProps = {
    component: forwardRef(
      (props, ref: React.Ref<HTMLAnchorElement>) => (
        <Link ref={ref as React.RefObject<HTMLAnchorElement>} {...props} to={item.url} target={itemTarget} />
      )
    )
  };
  ...
  return (
    ...
    >
      ...
      <ListItemText
        primary={
          <Typography variant={customization.isOpen.findIndex((id:number) => id === item.id) > -1 ? 'h5' : 'body1'} color="inherit">
            {item.title}
          </Typography>
        }
        secondary={
